...
void tgf_dead_time_2_1(int nrdm = 40, char *outfilename="./sim.root", int idoffset=0, float minFluence=0.05, float maxFluence=0.5, float minT50=0.001, float maxT50=0.04, int verbose = 0, int sourcetype=2, float rate= 20000., float aeff=150.)
{
    gROOT->Reset();
...
//    double simfluence = nevt / area; // simulated fluence
    ...
    switch (sourcetype) {
        ...
        case 3:
            nfile += data.Add("/Users/dub002/data/TGF/triggers_with_WWLLN/MC_simulations/time_simulation/TGF-daMC/TGF_10_Uniform.root");
            break;
           
        default:
            nfile += data.Add("/home/marisaldi/data/TGF41313-25_cutpl02-55.root");
            nfile += data.Add("/home/marisaldi/data/TGF41313-25_cutpl02-55bis.root");
            break;
    }
    ...
    for (int i=0; i<nrdm; i++) {
        ...
        switch (sourcetype) {
            ...
            case 1: // constant rate, uniform distribution in time, for validation of the DFE model by PB
            {
                ...
                printf("\n ----- Id = %8d Uniform distribution with rate = %.3f Hz, MCAL counts in %f ms interval = %d ----- \n", i, rate, twindow, nc);
                break;
            }
            ...
            case 2: // fake events on single bar, for validation, for validation of the FEE model by PB
            {
                ...
                atcal[0] = 0.;
                atcal[1] = (i+1)*tsep;
                printf("\n ----- Id = %8d Fake test for validation: n. of counts %d - t separation = %f ms ----- \n", i, nc, tsep);
                break;
            }
            ...
            case 3: // TGF time profile as simulated by D. Sarria and NØ, extract uniform random fluence, extract uniform entries for (energy,time) correlated values
            {
                ...
                sigma = t50*0.74;
                //nc = (int) (ncal*fluence/simfluence); // expected number of MCAL counts
                ...
                for (int h=0; h<nc; h++) {
                    ...
                    if (maxT50>0.) {
                        tbroad=rnd->Gaus(0.,sigma);
                        atcal[h] += tbroad ;
                    }

                }
                printf("\n ----- Id = %8d TGF Fluence = %.3f, MCAL counts = %d ----- \n", i, fluence, nc); //if (verbose)
                break;
            }
            ...
            default: // Gaussian shaped TGF, extract uniform random fluence and duration
            {
                ...
                printf("\n ----- Id = %8d TGF Fluence = %.3f, T50 = %.3f , MCAL counts = %d ----- \n", i, fluence, t50, nc); //if (verbose)
                break;
            }
        }
        ...
        for (int h=0; h<nc; h++) {
            switch (sourcetype) {
                case 2:
                    data.GetEntry(30); // 150: 20MeV; 120: 10MeV; 90: 5MeV; 60: 2MeV; 30: 1MeV; 0: 0.5MeV (entries in file fake.root made with macro make_fake_dataset.C)
                    break;
                    
                case 3:
                    int ievt = atsim[idc[h]];
                    data.GetEntry(ievt); // extract random event from simulations
                    break;
                  
                default:
                    int ievt = rnd->Integer(nevents);
                    data.GetEntry(ievt); // extract random event from simulations
                    break;
            }
